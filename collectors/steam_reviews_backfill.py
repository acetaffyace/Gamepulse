...
因此可以把「当前仍存在的全部评测」按创建日聚合，重建历史评测曲线，
不必等待逐日采集。

口径限制（必须随图表一起说明）：
  1. 只能看到「今天仍然存在」的评测。被删除、被隐藏或作者改为不公开的
     评测不会出现，因此回填出的某日新增评测数是该日评测的「存活数」，
     通常略低于当日真实值，且越久远的日期低估越多。
...
def aggregate_by_date(reviews: list[dict]) -> list[dict]:
    """按创建日聚合，并计算截至该日的累计好评率。"""
    ...
    out = []
    cum_total = cum_pos = 0
    for date_local in sorted(daily):
        slot = daily[date_local]
        ...
        out.append({
            ...
            "daily_review_rate": round(slot["new_positive"] / slot["new_reviews"] * 100, 2),
...
def main() -> int:
    ...
    if meta["total_expected"]:
        ...
        # 截断必须显式报出，否则会被当成完整历史使用
        if ratio < 95:
            print(f"\n  [警告] 覆盖率低于 95%，历史不完整。"
                  f"停止原因 {meta['stop_reason']}；"
                  f"若为 max_pages 请提高 --max-pages 后重跑。")
            meta["truncated"] = True

    daily = aggregate_by_date(reviews)
    if not daily:
        print("没有可聚合的评测")
        return 1

    write_json(RAW_DIR / "backfill" / f"steam_reviews_{args.game}.json",
               {"meta": meta, "collected_at": today_local(), "reviews": reviews})

    path = series_path(args.game, "review_history")
